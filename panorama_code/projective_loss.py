...
def disparity_loss(pred, src_tgt_stack, opt):
    depths, poses = pred
    src = src_tgt_stack[:,:-1]
    depths = [[1/d for d in dep] for dep in depths]
    ...
    for s in range(opt.num_scales):
        for i in range(opt.num_source):
            ...
            if (s != 0):
                scaled_src = tf.image.resize(scaled_src, depths[i][s].shape[1:3])
                target_im_scaled = tf.image.resize(target_im_scaled, depths[i][s].shape[1:3])
            curr_proj_image = projective_inverse_warp(
                scaled_src,
                tf.squeeze(depths[i][s], axis=3),
                poses[:,i],
                opt.intrinsics[:,:,s],
                do_wrap=opt.do_wrap,
