...
flags.DEFINE_string("saliency_report_path", "reports/saliency Report ", "the path to write the report on pruning success")
flags.DEFINE_string("run_name", "Saliency Curve", "the path to write the report on pruning success")
...
flags.DEFINE_integer("eval_steps", 5, "number of batches to use for evaluation. (None means all in training set)")
...
flags.DEFINE_integer("num_prunes", 30, "number of pruning steps to run")
flags.DEFINE_float("prune_rate", 0.001, "percentage of neurons to kill in a step")
...
def main(argv):
    '''
    test = tf.random.normal([5,2,2,6])
    flat = cylin.flatten()
    flat(test)
    print(flat.prune(None, tf.constant([1,2,3,5])))
    '''
    ...
    evaluate_saliency(prune_trainer, mnist_net, get_mnist_datset, 
            get_loss_categorical, grad2_saliency, FLAGS, "Grad2 "+FLAGS.run_name)

    evaluate_saliency(prune_trainer, mnist_net, get_mnist_datset, 
            get_loss_categorical, grad1_saliency, FLAGS, "Grad1 "+FLAGS.run_name)
    #FLAGS.initial_steps = 50
    #FLAGS.init_checkpoint_file = "grad1"+FLAGS.run_name+"saliency_base"

    evaluate_saliency(prune_trainer, mnist_net, get_mnist_datset, 
            get_loss_categorical, activity_saliency, FLAGS, "Activity "+FLAGS.run_name)
    evaluate_saliency(prune_trainer, mnist_net, get_mnist_datset, 
            get_loss_categorical, active_grad_saliency, FLAGS, "Grad1 Activity "+FLAGS.run_name)
    evaluate_saliency(prune_trainer, mnist_net, get_mnist_datset, 
            get_loss_categorical, l2_saliency, FLAGS, "L2 "+FLAGS.run_name)
    evaluate_saliency(prune_trainer, mnist_net, get_mnist_datset, 
            get_loss_categorical, l1_saliency, FLAGS, "L1"+FLAGS.run_name)
